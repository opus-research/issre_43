...
os.makedirs("../results/rq3/overall_guideline_summary", exist_ok=True)
os.makedirs("../results/rq3/plots", exist_ok=True)
...
for project in projects:
    ...
    try:
        with open(f'../results/rq2/{owner}_{repo}_guideline_compliance_output.json', 'r', encoding='utf-8') as f:
            compliance_data = json.load(f)
        with open(f'../data/pull_requests/{owner}_{repo}_aggregated_issues.json', 'r', encoding='utf-8') as f:
...
df_overall.to_csv("../results/rq3/overall_guideline_summary/overall_guideline_summary.csv", index=False)
df_overall.to_json("../results/rq3/overall_guideline_summary/overall_guideline_summary.json", orient="records", indent=2, force_ascii=False)
...
plt.legend()
plt.tight_layout()
plt.savefig("../results/rq3/plots/rq3_guideline_presence_percentages.png", dpi=300)
plt.close()
...
for language, counts_dict in language_counts.items():
    ...
    df_lang.to_csv(f"../results/rq3/overall_guideline_summary/{lang_safe}_guideline_summary.csv", index=False)
    df_lang.to_json(f"../results/rq3/overall_guideline_summary/{lang_safe}_guideline_summary.json", orient="records", indent=2, force_ascii=False)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"../results/rq3/plots/{lang_safe}_guideline_compliance_percentages.png", dpi=300)
    plt.close()
...
df_bin.to_csv("../results/rq3/overall_guideline_summary/binary_guideline_matrix.csv", index=False)
...
df_chi.to_csv("../results/rq3/overall_guideline_summary/chi_square_results.csv", index=False)
...
importance_df.to_csv("../results/rq3/overall_guideline_summary/random_forest_importance.csv", index=False)
...
plt.tight_layout()
plt.savefig("../results/rq3/plots/random_forest_importance.png", dpi=300)
plt.close()

# === Generate LaTeX Tables ===
latex_output_folder = "../results/rq3/latex_tables"
os.makedirs(latex_output_folder, exist_ok=True)

# Chi-Square LaTeX
with open(f"{latex_output_folder}/chi_square_table.tex", "w", encoding="utf-8") as f:
...
# Random Forest LaTeX
with open(f"{latex_output_folder}/random_forest_table.tex", "w", encoding="utf-8") as f:
